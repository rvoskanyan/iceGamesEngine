...
export const getFeedCsv = async (req, res) => {
  try {
    ...
    products.forEach((product, index) => {
      worksheet.addRow({row: `${index},,${product.name},${res.locals.websiteAddress}games/${product.alias},${res.locals.websiteAddress}${product.img},Купить игру ${product.name} c активацией в ${product.activationServiceId.name} со скидкой.,${product.priceTo},,RUB`});
    });
