...
mobileCartLinkNode &&
  mobileCartLinkNode.addEventListener('transitionend', () => {
...
if (successPaymentNode) {
  ...
  closeSuccessPaymentNode &&
    closeSuccessPaymentNode.addEventListener('click', () => {
      successPaymentNode.remove()
    })
}

reloadCheckFillUpStatusNode &&
  reloadCheckFillUpStatusNode.addEventListener('click', () => {
    document.location.reload()
  })

showAboutAsHomeNode &&
  showAboutAsHomeNode.addEventListener('click', () => {
...
openDescriptionSplitCatalogNode &&
  openDescriptionSplitCatalogNode.addEventListener('click', () => {
    ...
    const descriptionSplitCatalogCloseNode = document.querySelector(
      '.js-descriptionSplitCatalogClose'
    )
  ...
  const animate = () => {
    ...
    const int = setInterval(() => {
      ...
      if (currentValue < count) {
        return (counterAnimationNode.innerText =
          currentValue + step <= count ? currentValue + step : count)
      }
...
if (buyTurkeyPage) {
  ...
  if (emailVerifiedBuyTurkey) {
    ...
    const verificationCodeFieldFrame = emailVerifiedBuyTurkey.querySelector(
      '.js-verificationCodeFieldFrame'
    )
...
loadMoreSelectionsBtnNode &&
  loadMoreSelectionsBtnNode.addEventListener('click', async () => {
    ...
    const response = await postman.get(
      `${websiteAddress}api/selections?skip=${skip}&delistSelection=${delistSelection}`
    )
...
loadMoreRatingNode &&
  loadMoreRatingNode.addEventListener('click', async () => {
...
loadModeProductReviewsNode &&
  loadModeProductReviewsNode.addEventListener('click', async () => {
    ...
    const response = await postman.get(
      `${websiteAddress}api/reviews?skip=${skip}&targetId=${productId}&target=Product`
    )
    const result = await response.json()
    ...
    result.reviews.forEach(review => {
      ...
      <div class="item review">
          <div class="head">
              <a class="btn link userName" href="${websiteAddress}rating/${
        review.user.login
      }" title="Перейти на страницу ${review.user.login}">${review.user.login}</a>
          </div>
          <div class="grade">
...
loadMoreFillUpReviewsBtnNode &&
  loadMoreFillUpReviewsBtnNode.addEventListener('click', async () => {
    const skip = parseInt(loadMoreFillUpReviewsBtnNode.dataset.skip)

    const response = await postman.get(
      `${websiteAddress}api/reviews?skip=${skip}&target=FillUpSteam`
    )
    const result = await response.json()
    ...
    loadMoreFillUpReviewsBtnNode.dataset.skip =
      parseInt(loadMoreFillUpReviewsBtnNode.dataset.skip) + 5
  })

loadMoreReviewsBtnNode &&
  loadMoreReviewsBtnNode.addEventListener('click', async () => {
    ...
    result.reviews.forEach(review => {
      ...
      <div class="review">
        ...
        review.user.login
              ...
              <div class="forGame">Отзыв на игру: <a class="link gameName" href="${websiteAddress}games/${
        review.targetId.alias
      }">${review.targetId.name}</a></div>
          </div>
          <div class="grade">
              <span class="icon icon-star${review.eval >= 1 ? 'Fill' : ''}"></span>
              <span class="icon icon-star${review.eval >= 2 ? 'Fill' : ''}"></span>
...
whereLoginNode &&
  whereLoginNode.addEventListener('click', () => {
...
howToGetLoginNode &&
  howToGetLoginNode.addEventListener('click', () => {
    document.body.classList.add('noScrolling')
...
if (gamePageNode) {
  ...
  if (subscribeInStockBtnNode) {
    ...
    const sendSubscribe = async (email = null) => {
      ...
      const response = await postman.post(
        `${websiteAddress}api/products/${productId}/subscribeInStock`,
        params
      )
  ...
  addToCartBtnNode &&
    addToCartBtnNode.addEventListener('click', async () => {
      ...
      if (mobileCartLinkNode) {
        !mobileCartLinkNode.classList.contains('active') &&
          mobileCartLinkNode.classList.add('active')
    ...
    if (countProducts) {
        ...
        formConfirm.addEventListener('submit', async function (e) {
          ...
          switch (step) {
            ...
            case '2':
              ...
              demandConfirm = false
              if (payBtnNode.classList.contains('no-active'))
                payBtnNode.classList.remove('no-active')
      ...
      async function get_digiCheckout(products, isTwo = false) {
        ...
        for (const prd of products) {
          ...
          const responseAddCartDs = await fetch(
      ...
      function change_step(steps, step_id, prices, product_els, payment_button) {
        ...
        for (let i of steps.children) {
          ...
          } else if (i.classList.contains('payment-step-active'))
            i.classList.remove('payment-step-active')
        }
...
mobileSearchStringNode.addEventListener('input', async () => {
  ...
  if (!result.isLast) {
    searchResultNode.innerHTML += `
        <a class="goToAllSearchResults" href="/games${
          mobileSearchStringNode.value ? `?searchString=${mobileSearchStringNode.value}` : ''
        }">
            <div class="">
...
if (blogPageNode) {
  ...
  loadMoreNode.addEventListener('click', async () => {
    const response = await postman.get(
      `${websiteAddress}api/articles?skip=${loadMoreNode.dataset.skip}&limit=${countLoad}&includeFixed=false`
    )
    const result = await response.json()
    ...
    result.articles.forEach(article => {
      listArticlesNode.innerHTML += `
        <a class="article${article.rightImg ? ' right' : ''}" href="${websiteAddress}blog/${
        article.alias
      }" style="--bgColor: ${article.blockColor}">
            <img class="img" src="${websiteAddress}${
        article.img
      }" loading="lazy" alt="Изображение записи в блоге - ${article.name}">
            <div class="info">
                <div class="content">
...
if (catalogNode) {
  ...
  priceToNode.addEventListener(
    ...
    debounce(() => {
      ...
      let anotherValue = parseInt(
        priceFromNode.value ? priceFromNode.value : priceFromNode.dataset.min
      )
      if (anotherValue > value) {
  ...
  catalogNode.addEventListener('changeParams', async () => {
    ...
    const url = new URL(window.location.href)
    const queryUrl = `${websiteAddress}api/products${
      url.search ? url.search : '?'
    }&platform=${platform}&limit=${countLoad}${sectionName ? `&${sectionType}=${sectionName}` : ''}`
  ...
  async function loadMore(page) {
    ...
    const queryUrl = `${websiteAddress}api/products${
      url.search ? url.search : '?'
    }&platform=${platform}&skip=${skip}&limit=${countLoad}${
  ...
  async function pageActivation() {
    ...
    const activeObjects = pageObjects.filter(pageObject => {
      ...
      const bottomOffset =
        pageObject.node.getBoundingClientRect().top + pageObject.node.getBoundingClientRect().height
...
if (fillUpSteamFrom) {
  ...
  new AsyncForm({
    ...
    successHandler: (sendParams, results) => {
      ...
      window.open(
        `${websiteAddress}fill-up-steam/check-status?fillUpId=${results.fillUpId}`,
        '_self'
      )
    }
  })
